  ...
  return `
    <div class="row msg_container base_sent" data-message-id="${message.id}">
      ...
      <div class="col-md-2 col-xs-2 avatar">
          <img src="` + base_url + '/public/uploads/' + `${message.sender.UserPhoto}" width="50" height="50" class="img-responsive">
      </div>
  </div>
  `;
}
 ...
 * @param message
 */
function send(MessageReceiverId, message)
{
  let chat_box = $("#chat_box_" + MessageReceiverId);
  let chat_area = chat_box.find(".chat-area");
 
  $.ajax({
      url: base_url + "/send",
      data: {MessageReceiverId: MessageReceiverId, message: message, _token: $("meta[name='csrf-token']").attr("content")},
      method: "POST",
      dataType: "json",
      beforeSend: function () {
          if(chat_area.find(".loader").length  == 0) {
